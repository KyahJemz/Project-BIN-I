...
import {
	CreateContactDetailsRequestSchema,
	UpdateContactDetailsRequestSchema,
...
export async function POST(req: NextRequest) {
	...
	try {
		...
		const contactDetails =
			await contactDetailsService.createContactDetails(parsedRequest);
...
export async function GET(req: NextRequest) {
	...
	try {
		...
		if (id) {
			const contactDetails =
				await contactDetailsService.getContactDetailsById(id);
			return NextResponse.json(contactDetails);
		} else {
			const contactDetails =
				await contactDetailsService.getAllContactDetails();
			return NextResponse.json(contactDetails);
		}
	} catch (error: any) {
		const { statusCode, message } = ErrorResponses.UNHANDLED_ERROR(error);
	...
	try {
		if (id) {
			...
			return NextResponse.json({
				message: `Contact details with ID: ${id} deleted successfully`,
			});
		} else {
			const { statusCode, message } =
				ErrorResponses.MISSING_PARAMETER('id');
