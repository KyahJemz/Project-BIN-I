...
import { CreateEventRequestSchema, UpdateEventRequestSchema } from '@/validation/events.validation';
...
export async function PUT(req: NextRequest) {
    ...
    try {
        if (id) {
            await MongoDbConnect();
            const parsedRequest = UpdateEventRequestSchema.parse(await req.json());
            const eventService = new EventService(EventsModel);
            const updatedEvent = await eventService.updateEvent(id, parsedRequest);
...
export async function DELETE(req: NextRequest) {
    ...
    try {
        if (id) {
            ...
            return NextResponse.json({ message: `Event with ID: ${id} deleted successfully` });
        } else {
            const { statusCode, message } = ErrorResponses.MISSING_PARAMETER('id');
            return NextResponse.json({ message }, { status: statusCode });
        }
