...
import { CreateNewsRequestSchema, UpdateNewsRequestSchema } from '@/validation/news.validation';
...
export async function PUT(req: NextRequest) {
    ...
    try {
        if (id) {
            await MongoDbConnect();
            const parsedRequest = UpdateNewsRequestSchema.parse(await req.json());
            const newsService = new NewsService(NewsModel);
            const updatedEvent = await newsService.updateNews(id, parsedRequest);
...
export async function DELETE(req: NextRequest) {
    ...
    try {
        if (id) {
            ...
            return NextResponse.json({ message: `News with ID: ${id} deleted successfully` });
        } else {
            const { statusCode, message } = ErrorResponses.MISSING_PARAMETER('id');
            return NextResponse.json({ message }, { status: statusCode });
        }
