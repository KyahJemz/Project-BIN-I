...
import { CreateScheduleRequestSchema, UpdateScheduleRequestSchema } from '@/validation/schedule.validation';
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const parsedRequest = CreateScheduleRequestSchema.parse(await req.json());
...
export async function PUT(req: NextRequest) {
    ...
    try {
        if (id) {
            await MongoDbConnect();
            const parsedRequest = UpdateScheduleRequestSchema.parse(await req.json());
            const schedulesService = new ScheduleService(SchedulesModel);
            const updatedEvent = await schedulesService.updateSchedule(id, parsedRequest);
...
export async function DELETE(req: NextRequest) {
    ...
    try {
        if (id) {
            ...
            return NextResponse.json({ message: `Schedules with ID: ${id} deleted successfully` });
        } else {
            const { statusCode, message } = ErrorResponses.MISSING_PARAMETER('id');
            return NextResponse.json({ message }, { status: statusCode });
        }
