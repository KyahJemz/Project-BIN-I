...
import { CreateRoutesRequestSchema, UpdateRoutesRequestSchema } from '@/validation/routes.validation';
...
export async function PUT(req: NextRequest) {
    ...
    try {
        if (id) {
            await MongoDbConnect();
            const parsedRequest = UpdateRoutesRequestSchema.parse(await req.json());
            const routesService = new RoutesService(RoutesModel);
            const updatedEvent = await routesService.updateRoute(id, parsedRequest);
...
export async function DELETE(req: NextRequest) {
    ...
    try {
        if (id) {
            ...
            return NextResponse.json({ message: `Routes with ID: ${id} deleted successfully` });
        } else {
            const { statusCode, message } = ErrorResponses.MISSING_PARAMETER('id');
            return NextResponse.json({ message }, { status: statusCode });
        }
